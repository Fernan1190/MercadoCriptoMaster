...
// ✅ CORREGIDO: timestamp es NUMBER, no string
export interface Transaction { 
  ...
  amount: number; 
  price: number; 
  timestamp: number;  // ← CAMBIO: era string
  pnl?: number;       // ← AGREGADO: para compatibilidad con dominios
}
...
export interface Unit { 
  ...
  nextPathOptions?: { pathId: string; title: string; description: string; }[]; 
}
...
export interface CandleData { 
  timestamp: number;  // ← CAMBIO: era "time: string"
  open: number; 
  high: number; 
  low: number; 
  close: number; 
...
export interface MarketEvent { 
  ...
  impact: { BTC?: number; ETH?: number; SOL?: number; AAPL?: number; TSLA?: number; volatility: number; }; 
  duration: number; 
  icon: string; 
}
