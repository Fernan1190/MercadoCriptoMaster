...
const INITIAL_STATS: UserStats = {
  ...
  // INICIALIZACIÓN DE MINERÍA
  miningFarm: {
      racks: [],
      minedFragments: 0,
      totalHashrate: 0,
      totalPowerConsumption: 0,
      electricityCostPerWatt: 0.0001 // Costo por segundo por Watt (simulado)
  }
};

type SoundType = 'success' | 'error' | 'cash' | 'pop' | 'levelUp' | 'click' | 'chest' | 'news' | 'process' | 'meow' | 'fan';
...
export const GameProvider = ({ children }: { children: ReactNode }) => {
  ...
  const buyRack = useCallback((modelId: string) => {
      ...
      if (stats.balance >= rackModel.price) {
          ...
          setStats(prev => ({
              ...prev,
              balance: prev.balance - rackModel.price,
              miningFarm: {
                  ...prev.miningFarm,
                  racks: [...prev.miningFarm.racks, newRack]
              }
          }));
          playSound('success');
          return true;
  ...
  const buyMiner = useCallback((modelId: string, rackInstanceId: string) => {
      ...
      if (emptySlotIndex !== -1 && stats.balance >= minerModel.price) {
          ...
          setStats(prev => {
              ...
              return {
                  ...prev,
                  balance: prev.balance - minerModel.price,
                  miningFarm: {
                      ...currentFarm,
                      racks: newRacks,
                      totalHashrate: currentFarm.totalHashrate + minerModel.hashrate,
                      totalPowerConsumption: currentFarm.totalPowerConsumption + minerModel.power
                  }
              };
          });
          playSound('fan'); 
  ...
  const sellMinedCrypto = useCallback(() => {
      if (stats.miningFarm.minedFragments > 0) {
          ...
          setStats(prev => ({
              ...
              miningFarm: { ...prev.miningFarm, minedFragments: 0 }
          }));
          playSound('cash');
      }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // Lógica de Juego (Minería + Órdenes)
      setStats(prev => {
        const now = Date.now();
        ...
        if (miningFarm.totalHashrate > 0) {
            const production = miningFarm.totalHashrate * 0.0000001;
            const electricityCost = miningFarm.totalPowerConsumption * miningFarm.electricityCostPerWatt;
            balance -= electricityCost;
            if (balance < 0) balance = 0; 
            miningFarm = { ...miningFarm, minedFragments: miningFarm.minedFragments + production };
        }
  ...
  useEffect(() => {
    ...
    if (newUnlocked.length > 0) {
        ...
        setLatestAchievement(ACHIEVEMENTS.find(a => a.id === newUnlocked[0]) || null);
        playSound('levelUp');
    }
  }, [stats, playSound]);
  ...
  const updateStats = useCallback((xpGained: number, pathId?: PathId, levelIncrement: number = 0, perfectRun: boolean = false) => {
    setStats(prev => {
      ...
      next = { ...next, xp: next.xp + finalXp, masterCoins: next.masterCoins + coinsGained };
  ...
  const mineCoin = useCallback(() => { playSound('pop'); setStats(prev => ({ ...prev, masterCoins: prev.masterCoins + 1, minedCoins: (prev.minedCoins || 0) + 1 })); }, [playSound]);
  const buyAsset = useCallback((symbol: string, amount: number, currentPrice: number) => { const totalCost = amount * currentPrice; if (stats.balance >= totalCost) { const newTx: Transaction = { id: Date.now().toString(), type: 'buy', symbol, amount, price: currentPrice, timestamp: new Date().toLocaleString() }; setStats(prev => ({ ...prev, balance: prev.balance - totalCost, portfolio: { ...prev.portfolio, [symbol]: (prev.portfolio[symbol] || 0) + amount }, transactions: [newTx, ...(prev.transactions || [])] })); return true; } return false; }, [stats.balance]);
  const sellAsset = useCallback((symbol: string, amount: number, currentPrice: number) => { const currentQty = stats.portfolio[symbol] || 0; if (currentQty >= amount) { const totalGain = amount * currentPrice; const newTx: Transaction = { id: Date.now().toString(), type: 'sell', symbol, amount, price: currentPrice, timestamp: new Date().toLocaleString() }; setStats(prev => ({ ...prev, balance: prev.balance + totalGain, portfolio: { ...prev.portfolio, [symbol]: (prev.portfolio[symbol] || 0) - amount }, transactions: [newTx, ...(prev.transactions || [])] })); return true; } return false; }, [stats.portfolio]);
  ...
  const contextValue = useMemo(() => ({
    ...
    actions: { updateStats, mineCoin, buyAsset, sellAsset, deductHeart, buyHearts, useItem, addBookmark, stakeCoins, unstakeCoins, openChest, toggleTheme, updateNotes, getThemeClass, playSound, buyShopItem, buyTheme, equipTheme, saveLessonNote, recordAnswer, buyOfficeItem, unlockMarket, placeOrder, addCustomQuestion, activateBuff, changeOfficeSkin, buyRack, buyMiner, sellMinedCrypto }
  }), [stats, marketState, latestAchievement, latestEvent, updateStats, mineCoin, buyAsset, sellAsset, deductHeart, buyHearts, useItem, addBookmark, stakeCoins, unstakeCoins, openChest, toggleTheme, updateNotes, getThemeClass, playSound, buyShopItem, buyTheme, equipTheme, saveLessonNote, recordAnswer, buyOfficeItem, unlockMarket, placeOrder, addCustomQuestion, activateBuff, changeOfficeSkin, buyRack, buyMiner, sellMinedCrypto, clearEvent]);
