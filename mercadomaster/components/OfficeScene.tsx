...
const WeatherSystem = ({ trend }: { trend: 'up' | 'down' | 'neutral' }) => {
    const isRain = trend === 'down'; // Llueve si el mercado baja
    
    return (
        <group>
            {isRain ? (
                <>
                    {/* Tormenta (Mercado Bajista) */}
                ...
                <>
                    {/* Día Soleado (Mercado Alcista/Neutral) */}
                    <Cloud opacity={0.3} speed={0.2} bounds={[10, 2, 10]} segments={20} position={[-10, 15, -10]} color="white" />
                    <Cloud opacity={0.3} speed={0.2} bounds={[10, 2, 10]} segments={20} position={[15, 12, -15]} color="white" />
                </>
            )}
        </group>
    );
...
// --- COMPONENTES VISUALES DE MINERÍA (SIN CAMBIOS) ---
...
// ... (MUEBLES Y DECORACIÓN IGUAL QUE ANTES)
...
const OfficeScene: React.FC<OfficeVisualProps> = ({ level, items, skin, achievements, miningFarm }) => {
  ...
  return (
    <Canvas shadows dpr={[1, 2]} gl={{ antialias: true }}>
        ...
        <WeatherSystem trend={trend} /> {/* Clima Reactivo */}
